...
static View *init(View *self) {
  return (View *) $((ScrollHandle *) self, initWithFrame, NULL);
}
...
static bool captureEvent(Control *self, const SDL_Event *event) {
  ...
  ScrollHandle *this = (ScrollHandle *) self;
  ...
  if (event->type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
    ...
    if (self->state & ControlStateHighlighted) {
      if (this->delegate.didDrag) {
        this->delegate.didDrag(this, event->motion.yrel);
      }
      return true;
    }
...
static ScrollHandle *initWithFrame(ScrollHandle *self, const SDL_Rect *frame) {
  ...
  self = (ScrollHandle *) super(Control, self, initWithFrame, frame);
  if (self) {
    $((View *) self, addClassName, "handle");
  }
...
static void initialize(Class *clazz) {
  ...
  ((ScrollHandleInterface *) clazz->interface)->initWithFrame = initWithFrame;
}
