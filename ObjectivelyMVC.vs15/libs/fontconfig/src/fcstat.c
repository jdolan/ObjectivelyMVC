...
int FcStat(const FcChar8 *file, struct stat *statb) {
    ...
    rc = GetFullPathName((LPCSTR)file, sizeof(full_path_name), full_path_name,
                         &basename);
    if (rc == 0 || rc > sizeof(full_path_name)) return -1;

    rc = GetLongPathName(full_path_name, full_path_name,
    ...
    if (wfad.nFileSizeHigh > 0) return -1;
    statb->st_size = wfad.nFileSizeLow;

    statb->st_atime = (*(INT64 *)&wfad.ftLastAccessTime) / 10000000
                      - EPOCH_OFFSET;
...
static int FcDirChecksumScandirSorter(const void *a, const void *b) {
...
static int FcDirChecksum(const FcChar8 *dir, time_t *checksum) {
    ...
    while (n--) {
        ...
        if (dtype == DT_UNKNOWN) {
            ...
            if (S_ISDIR(statb.st_mode)) goto bail;
...
int FcStatChecksum(const FcChar8 *file, struct stat *statb) {
    if (FcStat(file, statb) == -1) return -1;
    ...
    if (FcIsFsMtimeBroken(file)) {
        if (FcDirChecksum(file, &statb->st_mtime) == -1) return -1;
    }
#endif
...
#endif
    if (p) {
        if (!flag && strcmp(p, "nfs") == 0) statb->is_remote_fs = FcTrue;
...
FcBool FcIsFsMmapSafe(int fd) {
    ...
    if (FcFStatFs(fd, &statb) < 0) return FcTrue;
