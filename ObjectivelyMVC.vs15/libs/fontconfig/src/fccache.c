...
#define CACHEBASE_LEN                                                          \
    (1 + 32 + 1 + sizeof(FC_ARCHITECTURE) + sizeof(FC_CACHE_SUFFIX))
...
static FcChar8 *FcDirCacheBasename(const FcChar8 *dir,
...
FcBool FcDirCacheUnlink(const FcChar8 *dir, FcConfig *config) {
    ...
    while ((cache_dir = FcStrListNext(list))) {
        ...
        if (!cache_hashed) break;
        (void)unlink((char *)cache_hashed);
        FcStrFree(cache_hashed);
    }
    FcStrListDone(list);
    /* return FcFalse if something went wrong */
    if (cache_dir) return FcFalse;
    return FcTrue;
}

static int FcDirCacheOpenFile(const FcChar8 *cache_file,
                              struct stat *file_stat) {
    int fd;

#ifdef _WIN32
    if (FcStat(cache_file, file_stat) < 0) return -1;
#endif
...
static FcBool FcDirCacheProcess(FcConfig *config, const FcChar8 *dir,
    ...
    if (FcStatChecksum(dir, &dir_stat) < 0) return FcFalse;
...
static void unlock_cache(void) { FcMutexUnlock(cache_lock); }
...
static FcCacheSkip *FcCacheFindByAddrUnlocked(void *object) {
     ...
     */
    for (i = fcCacheMaxLevel; --i >= 0;)
        while (next[i] && (char *)object
                          >= ((char *)next[i]->cache + next[i]->size))
            next = next[i]->next;
    /*
     * Here we are
     */
    s = next[0];
    if (s && (char *)object < ((char *)s->cache + s->size)) return s;
    return NULL;
}
...
void FcCacheObjectDereference(void *object) {
    ...
    if (skip) {
        if (FcRefDec(&skip->ref) == 1) FcDirCacheDisposeUnlocked(skip->cache);
    }
    unlock_cache();
}
...
void FcCacheFini(void) {
    ...
    for (i = 0; i < FC_CACHE_MAX_LEVEL; i++) assert(fcCacheChains[i] == NULL);
    assert(fcCacheMaxLevel == 0);

    free_lock();
}

static FcBool FcCacheTimeValid(FcCache *cache, struct stat *dir_stat) {
    struct stat dir_static;

    if (!dir_stat) {
        if (FcStatChecksum(FcCacheDir(cache), &dir_static) < 0) return FcFalse;
        dir_stat = &dir_static;
    }
 ...
 */
static FcCache *FcDirCacheMapFd(int fd, struct stat *fd_stat,
                                struct stat *dir_stat) {
    FcCache *cache;
    FcBool allocated = FcFalse;

    if (fd_stat->st_size < (int)sizeof(FcCache)) return NULL;
    cache = FcCacheFindByStat(fd_stat);
    if (cache) {
        if (FcCacheTimeValid(cache, dir_stat)) return cache;
...
#endif
        if (cache == MAP_FAILED) cache = NULL;
#elif defined(_WIN32)
        {
            HANDLE hFileMap;
    ...
    if (allocated) cache->magic = FC_CACHE_MAGIC_ALLOC;
...
void FcDirCacheReference(FcCache *cache, int nref) {
    ...
    if (skip) FcRefAdd(&skip->ref, nref);
}

void FcDirCacheUnload(FcCache *cache) { FcCacheObjectDereference(cache); }

static FcBool FcDirCacheMapHelper(int fd, struct stat *fd_stat,
                                  struct stat *dir_stat, void *closure) {
    FcCache *cache = FcDirCacheMapFd(fd, fd_stat, dir_stat);

    if (!cache) return FcFalse;
...
FcCache *FcDirCacheLoadFile(const FcChar8 *cache_file, struct stat *file_stat) {
    ...
    if (!file_stat) file_stat = &my_file_stat;
 ...
 */
static FcBool FcDirCacheValidateHelper(int fd, struct stat *fd_stat,
                                       struct stat *dir_stat,
...
FcCache *FcDirCacheBuild(FcFontSet *set, const FcChar8 *dir,
     ...
     * Directory name
     */
    if (!FcStrSerializeAlloc(serialize, dir)) goto bail1;
    /*
     * Subdirs
     */
    FcSerializeAlloc(serialize, dirs, dirs->num * sizeof(FcChar8 *));
    for (i = 0; i < dirs->num; i++)
        if (!FcStrSerializeAlloc(serialize, dirs->strs[i])) goto bail1;

    /*
     * Patterns
     */
    if (!FcFontSetSerializeAlloc(serialize, set)) goto bail1;
...
FcBool FcDirCacheWrite(FcCache *cache, FcConfig *config) {
    ...
    FcDirCacheBasename(dir, cache_base);
    cache_hashed = FcStrBuildFilename(cache_dir, cache_base, NULL);
    if (!cache_hashed) return FcFalse;
    FcStrFree(cache_dir);
    ...
    if (!atomic) goto bail1;

    if (!FcAtomicLock(atomic)) goto bail3;
    ...
    magic = cache->magic;
    if (magic != FC_CACHE_MAGIC_MMAP) cache->magic = FC_CACHE_MAGIC_MMAP;
    ...
    /* Switch magic back */
    if (magic != FC_CACHE_MAGIC_MMAP) cache->magic = magic;
    ...
    close(fd);
    if (!FcAtomicReplaceOrig(atomic)) goto bail4;
...
FcBool FcDirCacheClean(const FcChar8 *cache_dir, FcBool verbose) {
    ...
    while ((ent = readdir(d))) {
        ...
        if (strlen(ent->d_name) != 32
                                   + strlen("-" FC_ARCHITECTURE FC_CACHE_SUFFIX)
...
const FcChar8 *FcCacheDir args1(const FcCache *c) { return FcCacheDir(c); }
...
int FcCacheNumSubdir args1(const FcCache *c) { return c->dirs_count; }

int FcCacheNumFont args1(const FcCache *c) { return FcCacheSet(c)->nfont; }
...
void byteReverse(unsigned char *buf, unsigned longs) {
    FcChar32 t;
    do {
        t = (FcChar32)((unsigned)buf[3] << 8 | buf[2])
...
void FcCacheCreateTagFile(const FcConfig *config) {
    ...
    while ((cache_dir = FcStrListNext(list))) {
        ...
        if (FcDirCacheCreateTagFile(d)) break;
    }
    if (d) FcStrFree(d);
    FcStrListDone(list);
}
