...
static const struct {
    const char name[16];
    FcElement element;
} fcElementMap[] = { { "fontconfig", FcElementFontconfig },
                     { "dir", FcElementDir },
    ...
    if (parse) {
        if (parse->name)
            ...
            fprintf(stderr, "Fontconfig %s: line %d: ", s,
                    (int)XML_GetCurrentLineNumber(parse->parser));
        if (severe >= FcSevereError) parse->error = FcTrue;
    } else
...
static void FcTypecheckValue(FcConfigParse *parse, FcType value, FcType type) {
    if (value == FcTypeInteger) value = FcTypeDouble;
    if (type == FcTypeInteger) type = FcTypeDouble;
    if (value != type) {
        if ((value == FcTypeLangSet && type == FcTypeString)
            || (value == FcTypeString && type == FcTypeLangSet))
            return;
        if (type == FcTypeUnknown) return;
        /* It's perfectly fine to use user-define elements in expressions,
         * so don't warn in that case. */
        if (value == FcTypeUnknown) return;
...
static void FcTypecheckExpr(FcConfigParse *parse, FcExpr *expr, FcType type) {
    ...
    case FcOpField:
        ...
        if (o) FcTypecheckValue(parse, o->type, type);
        break;
    case FcOpConst:
...
static FcTest *FcTestCreate(FcConfigParse *parse, FcMatchKind kind, FcQual qual,
    ...
    if (test) {
        ...
        if (o) FcTypecheckExpr(parse, expr, o->type);
    }
    return test;
}
...
static FcVStack *FcVStackCreateAndPush(FcConfigParse *parse) {
    ...
    if (parse->vstack_static_used < sizeof(parse->vstack_static)
                                    / sizeof(parse->vstack_static[0]))
...
static void FcVStackPopAndDestroy(FcConfigParse *parse) {
    ...
    if (!vstack || vstack->pstack != parse->pstack) return;
...
static void FcVStackClear(FcConfigParse *parse) {
    while (FcVStackPeek(parse)) FcVStackPopAndDestroy(parse);
}
...
static FcChar8 **FcConfigSaveAttr(const XML_Char **attr, FcChar8 **buf,
    ...
    if (!attr) return 0;
    slen = 0;
    for (i = 0; attr[i]; i++) slen += strlen((char *)attr[i]) + 1;
    if (i == 0) return 0;
...
static FcBool FcPStackPush(FcConfigParse *parse, FcElement element,
    ...
    if (parse->pstack_static_used < sizeof(parse->pstack_static)
                                    / sizeof(parse->pstack_static[0]))
        new = &parse->pstack_static[parse->pstack_static_used++];
    else {
        new = malloc(sizeof(FcPStack));
        if (!new) return FcFalse;
    }
...
static FcBool FcPStackPop(FcConfigParse *parse) {
    ...
    if (old->attr && old->attr != old->attr_buf_static) free(old->attr);
...
static void FcParseBlank(FcConfigParse *parse) {
    ...
    while (n-- > 0) {
        ...
        if (!parse->config->blanks) {
            ...
            if (!parse->config->blanks) goto bail;
        }
        switch ((int)v->tag) {
        case FcVStackInteger:
            if (!FcBlanksAdd(parse->config->blanks, v->u.integer)) goto bail;
            break;
        case FcVStackRange:
            if (v->u.range.begin <= v->u.range.end) {
                for (i = v->u.range.begin; i <= v->u.range.end; i++) {
                    if (!FcBlanksAdd(parse->config->blanks, i)) goto bail;
                }
            }
            break;
        default:
...
static double FcStrtod(char *s, char **end) {
     ...
     */
    if ((dot = strchr(s, 0x2e)) && (locale_data = localeconv())
...
static void FcParseString(FcConfigParse *parse, FcVStackTag tag) {
    ...
    if (!FcVStackPushString(parse, tag, s)) FcStrFree(s);
}
    ...
    while ((vstack = FcVStackPeek(parse))) {
        ...
        case FcVStackInteger:
            if (!FcCharSetAddChar(charset, vstack->u.integer)) {
                FcConfigMessage(parse, FcSevereWarning,
                                "invalid character: 0x%04x", vstack->u.integer);
            } else
                n++;
            break;
        case FcVStackRange:
    ...
    while ((vstack = FcVStackPeek(parse))) {
        ...
        case FcVStackString:
            if (!FcLangSetAdd(langset, vstack->u.string)) {
                FcConfigMessage(parse, FcSevereWarning, "invalid langset: %s",
                                vstack->u.string);
            } else
                n++;
            break;
...
static void FcParseFamily(FcConfigParse *parse) {
    ...
    FcStrBufDestroy(&parse->pstack->str);
    if (expr) FcVStackPushExpr(parse, FcVStackFamily, expr);
}
...
static void FcParseAlias(FcConfigParse *parse) {
    ...
    while ((vstack = FcVStackPeek(parse))) {
        ...
        case FcVStackFamily:
            ...
            break;
        case FcVStackPrefer:
            if (prefer) FcExprDestroy(prefer);
            prefer = vstack->u.expr;
            vstack->tag = FcVStackNone;
            break;
        case FcVStackAccept:
            if (accept) FcExprDestroy(accept);
            accept = vstack->u.expr;
            vstack->tag = FcVStackNone;
            break;
        case FcVStackDefault:
            if (def) FcExprDestroy(def);
            def = vstack->u.expr;
            vstack->tag = FcVStackNone;
            break;
        case FcVStackTest:
    ...
    if (!prefer && !accept && !def) {
        ...
        FcTest *t = FcTestCreate(
            parse, FcMatchPattern, FcQualAny, (FcChar8 *)FC_FAMILY,
    ...
    if (prefer) {
        edit = FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpPrepend, prefer,
                            binding);
        if (!edit)
    ...
    if (accept) {
        edit = FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpAppend, accept,
                            binding);
        if (!edit)
    ...
    if (def) {
        edit = FcEditCreate(parse, FC_FAMILY_OBJECT, FcOpAppendLast, def,
                            binding);
        if (!edit)
            FcExprDestroy(def);
        else {
...
static void FcParseBinary(FcConfigParse *parse, FcOp op) {
    ...
    if (expr) FcVStackPushExpr(parse, FcVStackExpr, expr);
}
...
static void FcParseInclude(FcConfigParse *parse) {
    ...
    if (prefix) {
        ...
        if (FcFileIsDir(s)) {
        userdir:
            if (!userdir) userdir = FcStrdup(s);
        } else if (FcFileIsFile(s)) {
        userconf:
            if (!userconf) userconf = FcStrdup(s);
        } else {
    ...
    if (!FcConfigParseAndLoad(parse->config, s, !ignore_missing))
        ...
        if (deprecated == FcTrue && filename != NULL
            && !FcFileIsLink(filename)) {
            if (FcFileIsDir(filename)) {
                FcChar8 *parent = FcStrDirname(userdir);

                if (!FcFileIsDir(parent)) FcMakeDirectory(parent);
                FcStrFree(parent);
                if (FcFileIsDir(userdir) || rename((const char *)filename,
                                                   (const char *)userdir) != 0
                    || symlink((const char *)userdir, (const char *)filename)
                       != 0) {
                    if (!warn_confd) {
                        FcConfigMessage(parse, FcSevereWarning,
                                        "reading configurations from %s is "
                                        "deprecated. please move it to %s "
                                        "manually",
                                        s, userdir);
                        warn_confd = FcTrue;
                    }
        ...
        if (filename) FcStrFree(filename);
    }
#endif
    FcStrBufDestroy(&parse->pstack->str);
...
static FcOp FcConfigLexOp(const FcChar8 *op, const FcOpMap *map, int nmap) {
    ...
    for (i = 0; i < nmap; i++)
        if (!strcmp((char *)op, map[i].name)) return map[i].op;
    return FcOpInvalid;
}

static const FcOpMap fcCompareOps[] = { { "eq", FcOpEqual },
                                        { "not_eq", FcOpNotEqual },
                                        { "less", FcOpLess },
...
static void FcParseTest(FcConfigParse *parse) {
    ...
    if (iblanks_string) {
        ...
        if (f) flags |= FcOpFlagIgnoreBlanks;
    }
...
static const FcOpMap fcModeOps[] = { { "assign", FcOpAssign },
...
static void FcParseEdit(FcConfigParse *parse) {
    ...
    edit = FcEditCreate(parse, FcObjectFromName((char *)name), mode, expr,
                        binding);
    if (!edit) {
        FcConfigMessage(parse, FcSevereError, "out of memory");
        FcExprDestroy(expr);
    ...
    if (!FcVStackPushEdit(parse, edit)) FcEditDestroy(edit);
}
...
static void FcParseMatch(FcConfigParse *parse) {
    ...
    while ((vstack = FcVStackPeek(parse))) {
        ...
        case FcVStackTest:
            ...
            break;
        case FcVStackEdit:
            if (kind == FcMatchScan && vstack->u.edit->object
                                       > FC_MAX_BASE_OBJECT) {
                FcConfigMessage(parse, FcSevereError,
...
static void FcParseAcceptRejectFont(FcConfigParse *parse, FcElement element) {
    ...
    while ((vstack = FcVStackPeek(parse))) {
        switch ((int)vstack->tag) {
        case FcVStackGlob:
            if (!FcConfigGlobAdd(parse->config, vstack->u.string,
                                 element == FcElementAcceptfont)) {
                FcConfigMessage(parse, FcSevereError, "out of memory");
            }
            break;
        case FcVStackPattern:
            if (!FcConfigPatternsAdd(parse->config, vstack->u.pattern,
                                     element == FcElementAcceptfont)) {
                FcConfigMessage(parse, FcSevereError, "out of memory");
            } else
                vstack->tag = FcVStackNone;
...
static FcValue FcPopValue(FcConfigParse *parse) {
    ...
    if (!vstack) return value;

    switch ((int)vstack->tag) {
    case FcVStackString:
        value.u.s = FcStrdup(vstack->u.string);
        if (value.u.s) value.type = FcTypeString;
        break;
    case FcVStackConstant:
    ...
    case FcVStackCharSet:
        ...
        if (value.u.c) value.type = FcTypeCharSet;
        break;
    case FcVStackLangSet:
        value.u.l = FcLangSetCopy(vstack->u.langset);
        if (value.u.l) value.type = FcTypeLangSet;
        break;
    default:
...
static void FcParsePatelt(FcConfigParse *parse) {
    ...
    for (;;) {
        ...
        if (value.type == FcTypeVoid) break;
...
static FcBool FcConfigParseAndLoadDir(FcConfig *config, const FcChar8 *name,
    ...
    if (FcDebug() & FC_DBG_CONFIG) printf("\tScanning config dir %s\n", dir);
...
FcBool FcConfigParseAndLoad(FcConfig *config, const FcChar8 *name,
    ...
    if (!FcConfigParseInit(&parse, name, config, p)) goto bail2;
...
bail0:
    if (error && complain) {
        if (name)
            FcConfigMessage(0, FcSevereError, "Cannot load config file \"%s\"",
                            name);
        else
