...
FcValueListPtr FcValueListPrepend(FcValueListPtr vallist, FcValue value,
    ...
    if (value.type == FcTypeVoid) return vallist;
    new = FcValueListCreate();
    if (!new) return vallist;
...
FcBool FcValueEqual(FcValue va, FcValue vb) {
    if (va.type != vb.type) {
        ...
        if (va.type != vb.type) return FcFalse;
    }
    switch (va.type) {
    case FcTypeUnknown:
...
static FcBool FcValueListEqual(FcValueListPtr la, FcValueListPtr lb) {
    if (la == lb) return FcTrue;

    while (la && lb) {
        if (!FcValueEqual(la->value, lb->value)) return FcFalse;
        la = FcValueListNext(la);
        lb = FcValueListNext(lb);
    }
    if (la || lb) return FcFalse;
    return FcTrue;
}
...
FcPatternElt *FcPatternObjectInsertElt(FcPattern *p, FcObject object) {
    ...
    if (i < 0) {
        ...
        if (p->num + 1 >= p->size) {
            ...
            if (p->size) {
                ...
                if (!e) /* maybe it was mmapped */
                {
                    ...
                    if (e) memcpy(e, e0, p->num * sizeof(FcPatternElt));
                }
            } else
        ...
        e = FcPatternElts(p);
        /* move elts up */
        memmove(e + i + 1, e + i, sizeof(FcPatternElt) * (p->num - i));
...
FcBool FcPatternEqual(const FcPattern *pa, const FcPattern *pb) {
    ...
    if (pa == pb) return FcTrue;

    if (pa->num != pb->num) return FcFalse;
    pae = FcPatternElts(pa);
    pbe = FcPatternElts(pb);
    for (i = 0; i < pa->num; i++) {
        if (pae[i].object != pbe[i].object) return FcFalse;
...
FcBool FcPatternObjectListAdd(FcPattern *p, FcObject object,
    ...
    if (FcRefIsConst(&p->ref)) goto bail0;
...
FcBool FcPatternObjectAddWithBinding(FcPattern *p, FcObject object,
    ...
    if (value.type == FcTypeVoid) goto bail1;
...
FcBool FcPatternObjectDel(FcPattern *p, FcObject object) {
    ...
    memmove(e, e + 1,
            (FcPatternElts(p) + p->num - (e + 1)) * sizeof(FcPatternElt));
    p->num--;
    ...
    for (prev = &e->values; (l = *prev); prev = &l->next) {
        if (!id) {
            *prev = l->next;
            ...
            if (!e->values) FcPatternDel(p, object);
            return FcTrue;
        }
        id--;
    }
...
FcBool FcPatternObjectAddString(FcPattern *p, FcObject object,
                                const FcChar8 *s) {
    FcValue v;
...
FcResult FcPatternObjectGetString(const FcPattern *p, FcObject object, int id,
    ...
    r = FcPatternObjectGet(p, object, id, &v);
    if (r != FcResultMatch) return r;
    if (v.type != FcTypeString) return FcResultTypeMismatch;
...
FcResult FcPatternGetBool(const FcPattern *p, const char *object, int id,
    ...
    if (r != FcResultMatch) return r;
    if (v.type != FcTypeBool) return FcResultTypeMismatch;
    *b = v.u.b;
    return FcResultMatch;
}
...
FcResult FcPatternGetCharSet(const FcPattern *p, const char *object, int id,
    ...
    r = FcPatternGet(p, object, id, &v);
    if (r != FcResultMatch) return r;
    if (v.type != FcTypeCharSet) return FcResultTypeMismatch;
    *c = (FcCharSet *)v.u.c;
    return FcResultMatch;
}
...
FcResult FcPatternGetFTFace(const FcPattern *p, const char *object, int id,
    ...
    r = FcPatternGet(p, object, id, &v);
    if (r != FcResultMatch) return r;
    if (v.type != FcTypeFTFace) return FcResultTypeMismatch;
    *f = (FT_Face)v.u.f;
    return FcResultMatch;
}
...
FcResult FcPatternGetLangSet(const FcPattern *p, const char *object, int id,
    ...
    r = FcPatternGet(p, object, id, &v);
    if (r != FcResultMatch) return r;
    if (v.type != FcTypeLangSet) return FcResultTypeMismatch;
    *ls = (FcLangSet *)v.u.l;
    return FcResultMatch;
}
...
FcPattern *FcPatternFilter(FcPattern *p, const FcObjectSet *os) {
    ...
    if (!os) return FcPatternDuplicate(p);
...
FcBool FcPatternSerializeAlloc(FcSerialize *serialize, const FcPattern *pat) {
    ...
    if (!FcSerializeAlloc(serialize, pat, sizeof(FcPattern))) return FcFalse;
...
FcPattern *FcPatternSerialize(FcSerialize *serialize, const FcPattern *pat) {
    ...
    *pat_serialized = *pat;
    ...
    for (i = 0; i < pat->num; i++) {
        ...
        if (!values_serialized) return NULL;
...
FcBool FcValueListSerializeAlloc(FcSerialize *serialize,
                                 const FcValueList *vl) {
    while (vl) {
        if (!FcSerializeAlloc(serialize, vl, sizeof(FcValueList)))
            return FcFalse;
        switch ((int)vl->value.type) {
        case FcTypeString:
            if (!FcStrSerializeAlloc(serialize, vl->value.u.s)) return FcFalse;
            break;
        case FcTypeCharSet:
...
FcValueList *FcValueListSerialize(FcSerialize *serialize,
                                  const FcValueList *vl) {
