...
static void FcLangSetBitSet(FcLangSet *ls, unsigned int id) {
    ...
    bucket = id >> 5;
    if (bucket >= ls->map_size) return; /* shouldn't happen really */
...
static FcBool FcLangSetBitGet(const FcLangSet *ls, unsigned int id) {
    ...
    id = fcLangCharSetIndices[id];
    bucket = id >> 5;
    if (bucket >= ls->map_size) return FcFalse;
...
FcLangSet *FcFreeTypeLangSet(const FcCharSet *charset,
    ...
    if (exclusiveLang) exclusiveCharset = FcLangGetCharSet(exclusiveLang);
    ls = FcLangSetCreate();
    if (!ls) return 0;
    ...
    for (i = 0; i < NUM_LANG_CHAR_SET; i++) {
        ...
        if (!missing) FcLangSetBitSet(ls, i);
    }
...
FcChar8 *FcLangNormalize(const FcChar8 *lang) {
    ...
    if (!territory) territory = strchr((const char *)s, '-');
    if (territory) {
        *territory = 0;
        territory++;
    ...
    if (territory) {
        ...
        if (FcLangSetIndex(s) < 0) {
            memmove(territory - 1, territory + tlen,
                    (mlen > 0 ? mlen + 1 : 0) + 1);
            if (modifier) modifier = territory;
        } else {
            result = s;
...
const FcCharSet *FcLangGetCharSet(const FcChar8 *lang) {
    ...
    for (i = 0; i < NUM_LANG_CHAR_SET; i++) {
        ...
        case FcLangDifferentTerritory:
            if (country == -1) country = i;
        case FcLangDifferentLang:
        default:
            break;
        }
    }
    if (country == -1) return 0;
...
void FcLangSetDestroy(FcLangSet *ls) {
    if (ls->extra) FcStrSetDestroy(ls->extra);
    free(ls);
}
...
static int FcLangSetIndex(const FcChar8 *lang) {
    ...
    if (firstChar < 'a') {
        ...
        /* no matches */
        if (low > high) return -low; /* next entry after where it would be */
    }
    ...
    while (low <= high) {
        ...
        else { /* fast path for resolving 2-letter languages (by far the most
                * common) after
                * finding the first char (probably already true because of the
                * hash table) */
...
FcLangResult FcLangSetHasLang(const FcLangSet *ls, const FcChar8 *lang) {
    ...
    for (i = id - 1; i >= 0; i--) {
        ...
        if (r == FcLangDifferentLang) break;
        if (FcLangSetBitGet(ls, i) && r < best) best = r;
    }
...
FcLangResult FcLangSetCompare(const FcLangSet *lsa, const FcLangSet *lsb) {
    ...
    for (i = 0; i < count; i++)
        if (lsa->map[i] & lsb->map[i]) return FcLangEqual;
    best = FcLangDifferentLang;
...
FcChar32 FcLangSetHash(const FcLangSet *ls) {
    ...
    for (i = 0; i < count; i++) h ^= ls->map[i];
    if (ls->extra) h ^= ls->extra->num;
    return h;
}
...
FcLangSet *FcNameParseLangSet(const FcChar8 *string) {
    ...
    for (;;) {
        for (i = 0; i < 31; i++) {
            c = *string++;
            if (c == '\0' || c == '|') break; /* end of this code */
            lang[i] = c;
        }
        lang[i] = '\0';
        if (!FcLangSetAdd(ls, lang)) goto bail1;
        if (c == '\0') break;
    }
    return ls;
bail1:
        ...
        if ((bits = ls->map[i])) {
            for (bit = 0; bit <= 31; bit++)
                if (bits & (1 << bit)) {
                    int id = (i << 5) | bit;
                    if (!first)
                        if (!FcStrBufChar(buf, '|')) return FcFalse;
                    if (!FcStrBufString(
                             buf,
...
FcBool FcLangSetEqual(const FcLangSet *lsa, const FcLangSet *lsb) {
    ...
    count = FC_MIN(NUM_LANG_SET_MAP, count);
    for (i = 0; i < count; i++) {
        if (lsa->map[i] != lsb->map[i]) return FcFalse;
    }
    if (!lsa->extra && !lsb->extra) return FcTrue;
    if (lsa->extra && lsb->extra) return FcStrSetEqual(lsa->extra, lsb->extra);
    return FcFalse;
}
...
static FcBool FcLangSetContainsLang(const FcLangSet *ls, const FcChar8 *lang) {
    ...
    if (ls->extra) {
        ...
        if (list) {
            while ((extra = FcStrListNext(list))) {
                if (FcLangContains(extra, lang)) break;
            }
            FcStrListDone(list);
            if (extra) return FcTrue;
        }
    }
    return FcFalse;
}
...
FcBool FcLangSetSerializeAlloc(FcSerialize *serialize, const FcLangSet *l) {
    if (!FcSerializeAlloc(serialize, l, sizeof(FcLangSet))) return FcFalse;
    return FcTrue;
}
...
FcStrSet *FcLangSetGetLangs(const FcLangSet *ls) {
    ...
    for (i = 0; i < NUM_LANG_CHAR_SET; i++)
        if (FcLangSetBitGet(ls, i)) FcStrSetAdd(langs, fcLangCharSets[i].lang);
    ...
    if (ls->extra) {
        ...
        if (list) {
            while ((extra = FcStrListNext(list))) FcStrSetAdd(langs, extra);
