...
intptr_t FcAlignSize(intptr_t size) {
    ...
    if (rem) size += sizeof(FcAlign) - rem;
    return size;
}
...
void FcSerializeDestroy(FcSerialize *serialize) {
    ...
    if (serialize->cs_freezer) FcCharSetFreezerDestroy(serialize->cs_freezer);
    free(serialize);
}
...
FcBool FcSerializeAlloc(FcSerialize *serialize, const void *object, int size) {
    ...
    for (buck = serialize->buckets[bucket]; buck; buck = buck->next)
        if (buck->object == object) return FcTrue;
    buck = malloc(sizeof(FcSerializeBucket));
    if (!buck) return FcFalse;
    buck->object = object;
...
intptr_t FcSerializeOffset(FcSerialize *serialize, const void *object) {
    ...
    for (buck = serialize->buckets[bucket]; buck; buck = buck->next)
        if (buck->object == object) return buck->offset;
    return 0;
}
