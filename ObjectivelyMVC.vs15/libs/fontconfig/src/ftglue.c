...
static FT_Pointer ftglue_qalloc(FT_Memory memory, FT_ULong size,
    ...
    if (size > 0) {
        ...
        if (!block) error = FT_Err_Out_Of_Memory;
    }
...
static void ftglue_free(FT_Memory memory, FT_Pointer block) {
    if (block) memory->free(memory, block);
}
...
ftglue_stream_frame_enter(FT_Stream stream, FT_ULong count) {
    ...
    if (stream->read) {
        ...
        if (QALLOC(stream->base, count)) goto Exit;
...
ftglue_face_goto_table(FT_Face face, FT_ULong the_tag, FT_Stream stream) {
    ...
    if (!FT_IS_SFNT(face)) {
        ...
        if (FILE_Seek(0) || ACCESS_Frame(4)) goto Exit;
        ...
        if (FILE_Seek(offset + 4) || ACCESS_Frame(2)) goto Exit;

        count = GET_UShort();

        FORGET_Frame();

        if (FILE_Seek(offset + 12) || ACCESS_Frame(count * 16)) goto Exit;
