...
FcBool FcConfigInit(void) { return FcConfigEnsure() ? FcTrue : FcFalse; }
...
FcConfig *FcConfigCreate(void) {
    ...
    if (!config->configDirs) goto bail1;

    config->configFiles = FcStrSetCreate();
    if (!config->configFiles) goto bail2;

    config->fontDirs = FcStrSetCreate();
    if (!config->fontDirs) goto bail3;

    config->acceptGlobs = FcStrSetCreate();
    if (!config->acceptGlobs) goto bail4;

    config->rejectGlobs = FcStrSetCreate();
    if (!config->rejectGlobs) goto bail5;

    config->acceptPatterns = FcFontSetCreate();
    if (!config->acceptPatterns) goto bail6;

    config->rejectPatterns = FcFontSetCreate();
    if (!config->rejectPatterns) goto bail7;

    config->cacheDirs = FcStrSetCreate();
    if (!config->cacheDirs) goto bail8;
...
FcBool FcConfigUptoDate(FcConfig *config) {
    ...
    if (!config) {
        ...
        if (!config) return FcFalse;
    }
    config_time = FcConfigNewestFile(config->configFiles);
    config_dir_time = FcConfigNewestFile(config->configDirs);
    font_time = FcConfigNewestFile(config->fontDirs);
    if ((config_time.set && config_time.time - config->rescanTime > 0)
        || (config_dir_time.set && (config_dir_time.time - config->rescanTime)
                                   > 0)
...
static void FcSubstDestroy(FcSubst *s) {
    FcSubst *n;

    while (s) {
        n = s->next;
        if (s->rule) FcRuleDestroy(s->rule);
        free(s);
        s = n;
    }
}
...
FcExpr *FcConfigAllocExpr(FcConfig *config) {
    if (!config->expr_pool || config->expr_pool->next
                              == config->expr_pool->end) {
        FcExprPage *new_page;
...
void FcConfigDestroy(FcConfig *config) {
    ...
    if (FcRefDec(&config->ref) != 1) return;
    ...
    if (config->blanks) FcBlanksDestroy(config->blanks);

    FcSubstDestroy(config->substPattern);
    FcSubstDestroy(config->substFont);
    FcSubstDestroy(config->substScan);
    for (set = FcSetSystem; set <= FcSetApplication; set++)
        if (config->fonts[set]) FcFontSetDestroy(config->fonts[set]);
    ...
    if (config->sysRoot) FcStrFree(config->sysRoot);
...
FcBool FcConfigAddCache(FcConfig *config, FcCache *cache, FcSetName set,
    ...
    if (fs) {
        ...
        for (i = 0; i < fs->nfont; i++) {
             ...
             */
            if (!FcConfigAcceptFont(config, font)) continue;

            if (FcFontSetAdd(config->fonts[set], font)) nref++;
        }
...
static FcBool FcConfigAddDirList(FcConfig *config, FcSetName set,
    ...
    if (!dirlist) return FcFalse;

    while ((dir = FcStrListNext(dirlist))) {
        if (FcDebug() & FC_DBG_FONTSET) printf("adding fonts from%s\n", dir);
...
FcBool FcConfigBuildFonts(FcConfig *config) {
    ...
    if (!fonts) return FcFalse;

    FcConfigSetFonts(config, fonts, FcSetSystem);

    if (!FcConfigAddDirList(config, FcSetSystem, config->fontDirs))
        return FcFalse;
    if (FcDebug() & FC_DBG_FONTSET) FcFontSetPrint(fonts);
    return FcTrue;
}
...
retry:
    ...
    if (config == cfg) return FcTrue;

    if (config && !config->fonts[FcSetSystem])
        if (!FcConfigBuildFonts(config)) return FcFalse;

    if (!fc_atomic_ptr_cmpexch(&_fcConfig, cfg, config)) goto retry;
...
FcConfig *FcConfigGetCurrent(void) { return FcConfigEnsure(); }
...
FcBool FcConfigAddDir(FcConfig *config, const FcChar8 *d) {
    return (FcConfigAddConfigDir(config, d) && FcConfigAddFontDir(config, d));
}
...
FcChar8 *FcConfigGetCache(FcConfig *config FC_UNUSED) { return NULL; }
...
FcBool FcConfigAddBlank(FcConfig *config, FcChar32 blank) {
    ...
    if (!FcBlanksAdd(b, blank)) {
        if (freeme) FcBlanksDestroy(freeme);
        return FcFalse;
    }
    config->blanks = b;
    return FcTrue;
...
FcBool FcConfigAddRule(FcConfig *config, FcRule *rule, FcMatchKind kind) {
    ...
    *prev = subst;
    ...
    for (r = rule; r; r = r->next) {
        switch (r->type) {
        case FcRuleTest:
            if (r->u.test && r->u.test->kind == FcMatchDefault)
                r->u.test->kind = kind;

            if (n < r->u.test->object) n = r->u.test->object;
            break;
        case FcRuleEdit:
            if (n < r->u.edit->object) n = r->u.edit->object;
            break;
        default:
            break;
        }
    ...
    if (config->maxObjects < n) config->maxObjects = n;
...
FcBool FcConfigCompareValue(const FcValue *left_o, unsigned int op_,
    ...
    if (left.type == right.type) {
        ...
        if (op == FcOpNotEqual || op == FcOpNotContains) ret = FcTrue;
    }
    return ret;
}
...
static FcValue FcConfigEvaluate(FcPattern *p, FcPattern *p_pat,
    ...
    case FcOpField:
        if (kind == FcMatchFont && e->u.name.kind == FcMatchPattern) {
            ...
            fprintf(stderr, "Fontconfig warning: <name> tag has "
                            "target=\"font\" in a <match "
                            "target=\"pattern\">.\n");
            v.type = FcTypeVoid;
        } else {
...
static FcBool FcConfigAdd(FcValueListPtr *head, FcValueList *position,
    ...
    for (v = new; v != NULL; v = FcValueListNext(v))
        if (v->binding == FcValueBindingSame) v->binding = sameBinding;
    if (append) {
        if (position)
            prev = &position->next;
        else
        ...
        if (position) {
            for (prev = head; *prev != NULL; prev = &(*prev)->next) {
                if (*prev == position) break;
            }
        } else
            prev = head;

        if (FcDebug() & FC_DBG_EDIT) {
            if (*prev == NULL) printf("position not on list\n");
        }
    }
...
static void FcConfigPatternDel(FcPattern *p, FcObject object) {
    ...
    if (!e) return;
    while (e->values != NULL) FcConfigDel(&e->values, e->values);
}

static void FcConfigPatternCanon(FcPattern *p, FcObject object) {
    FcPatternElt *e = FcPatternObjectFindElt(p, object);
    if (!e) return;
    if (e->values == NULL) FcPatternObjectDel(p, object);
}
...
FcBool FcConfigSubstituteWithPat(FcConfig *config, FcPattern *p,
    ...
    case FcMatchPattern:
        ...
        if (FcPatternObjectGet(p, FC_PRGNAME_OBJECT, 0, &v)
            == FcResultNoMatch) {
    ...
    for (; s; s = s->next) {
        ...
        for (; r; r = r->next) {
            ...
            case FcRuleTest:
                ...
                if (!e) {
                    if (r->u.test->qual == FcQualAll) {
                        ...
                        if (FcDebug() & FC_DBG_EDIT) printf("No match\n");
                        goto bail;
                    }
                }
                /*
                ...
                if (!value[object] && kind == r->u.test->kind)
                    value[object] = vl;
                if (!vl || (r->u.test->qual == FcQualFirst && vl != e->values)
            ...
            case FcRuleEdit:
                ...
                case FcOpPrepend:
                    if (value[object]) {
                        FcConfigAdd(&elt[object]->values, value[object],
                                    FcFalse, l, r->u.edit->object);
                        break;
                    }
                /* fall through ... */
                ...
                case FcOpAppend:
                    if (value[object]) {
                        FcConfigAdd(&elt[object]->values, value[object], FcTrue,
                                    l, r->u.edit->object);
                        break;
                    }
                /* fall through ... */
...
/* make sure there's a single separator */
    ...
    if (access((char *)path, R_OK) == 0) return path;
...
static FcChar8 **FcConfigGetPath(void) {
    ...
    if (env) {
        ...
        npath++;
        while (*e)
            if (*e++ == FC_SEARCH_PATH_SEPARATOR) npath++;
    }
    ...
    if (env) {
        ...
        while (*e) {
            ...
            if (!colon) colon = e + strlen((char *)e);
            path[i] = malloc(colon - e + 1);
            if (!path[i]) goto bail1;
...
FcChar8 *FcConfigFilename(const FcChar8 *url) {
    ...
    if (!url || !*url) {
        ...
        if (!url) url = (FcChar8 *)FONTCONFIG_FILE;
    }
    file = 0;

#ifdef _WIN32
    if (isalpha(*url) && url[1] == ':' && (url[2] == '/' || url[2] == '\\'))
        goto absolute_path;
#endif
...
static FcBool FcConfigGlobsMatch(const FcStrSet *globs, const FcChar8 *string) {
    ...
    for (i = 0; i < globs->num; i++)
        if (FcStrGlobMatch(globs->strs[i], string)) return FcTrue;
    return FcFalse;
}

FcBool FcConfigAcceptFilename(FcConfig *config, const FcChar8 *filename) {
    if (FcConfigGlobsMatch(config->acceptGlobs, filename)) return FcTrue;
    if (FcConfigGlobsMatch(config->rejectGlobs, filename)) return FcFalse;
    return FcTrue;
}
...
FcBool FcConfigPatternsAdd(FcConfig *config, FcPattern *pattern,
                           FcBool accept) {
...
static FcBool FcConfigPatternsMatch(const FcFontSet *patterns,
    ...
    for (i = 0; i < patterns->nfont; i++)
        if (FcListPatternMatchAny(patterns->fonts[i], font)) return FcTrue;
    return FcFalse;
}

FcBool FcConfigAcceptFont(FcConfig *config, const FcPattern *font) {
    if (FcConfigPatternsMatch(config->acceptPatterns, font)) return FcTrue;
    if (FcConfigPatternsMatch(config->rejectPatterns, font)) return FcFalse;
    return FcTrue;
}
