        ...
        while (min <= max) {
            ...
            if (high <= ucs4)
                ...
                case FC_CASE_FOLD_EVEN_ODD:
                    if ((ucs4 & 1) != (fold->upper & 1)) return r;
                /* fall through ... */
                default:
...
static FcChar8 FcStrCaseWalkerNext(FcCaseWalker *w, const char *delims) {
    ...
    if ((r & 0xc0) == 0xc0) return FcStrCaseWalkerLong(w, r);
    if ('A' <= r && r <= 'Z') r = r - 'A' + 'a';
    return r;
}
...
FcChar8 *FcStrDowncase(const FcChar8 *s) {
    ...
    while (FcStrCaseWalkerNext(&w, NULL)) len++;
    d = dst = malloc(len + 1);
    if (!d) return 0;
    FcStrCaseWalkerInit(s, &w);
...
#ifdef USE_REGEX
static FcBool _FcStrRegexCmp(const FcChar8 *s, const FcChar8 *regex, int cflags,
                             int eflags) {
    int ret = -1;
    regex_t reg;
...
FcBool FcStrRegexCmp(const FcChar8 *s, const FcChar8 *regex) {
    return _FcStrRegexCmp(s, regex, REG_EXTENDED | REG_NOSUB, 0);
}

FcBool FcStrRegexCmpIgnoreCase(const FcChar8 *s, const FcChar8 *regex) {
...
FcChar32 FcStrHashIgnoreCase(const FcChar8 *s) {
    ...
    FcStrCaseWalkerInit(s, &w);
    while ((c = FcStrCaseWalkerNext(&w, NULL))) h = ((h << 3) ^ (h >> 3)) ^ c;
    return h;
}
...
static FcBool FcStrIsAtIgnoreBlanksAndCase(const FcChar8 *s1,
                                           const FcChar8 *s2) {
    FcCaseWalker w1, w2;
    FcChar8 c1, c2;
...
const FcChar8 *FcStrContainsIgnoreBlanksAndCase(const FcChar8 *s1,
                                                const FcChar8 *s2) {
    while (*s1) {
        if (FcStrIsAtIgnoreBlanksAndCase(s1, s2)) return s1;
        s1++;
    }
    return 0;
}
...
static FcBool FcCharIsPunct(const FcChar8 c) {
    if (c < '0') return FcTrue;
    if (c <= '9') return FcFalse;
    if (c < 'A') return FcTrue;
    if (c <= 'Z') return FcFalse;
    if (c < 'a') return FcTrue;
    if (c <= 'z') return FcFalse;
    if (c <= '~') return FcTrue;
    return FcFalse;
...
const FcChar8 *FcStrContainsIgnoreCase(const FcChar8 *s1, const FcChar8 *s2) {
    while (*s1) {
        if (FcStrIsAtIgnoreCase(s1, s2)) return s1;
        s1++;
    }
    return 0;
}
...
const FcChar8 *FcStrContainsWord(const FcChar8 *s1, const FcChar8 *s2) {
    ...
    while (s1len >= s2len) {
        ...
        wordStart = FcFalse;
        if (FcCharIsPunct(*s1)) wordStart = FcTrue;
        s1++;
        s1len--;
    }
    return 0;
    ...
    while ((c = *glob++)) {
        ...
        case '*':
            ...
            if (strchr((char *)glob, '*') == 0) {
                ...
                l2 = strlen((char *)glob);
                if (l1 < l2) return FcFalse;
                string += (l1 - l2);
            }
            while (*string) {
                if (FcStrGlobMatch(glob, string)) return FcTrue;
                string++;
            }
            return FcFalse;
        case '?':
            if (*string++ == '\0') return FcFalse;
            break;
        default:
            if (*string++ != c) return FcFalse;
            break;
        }
    }
    return *string == '\0';
                  ...
                  int len) /* in bytes */
{
    ...
    if ((a & 0xfc00) == 0xd800) {
        ...
        result = ((((FcChar32)a & 0x3ff) << 10) | ((FcChar32)b & 0x3ff))
                 + 0x10000;
    } else
        result = a;
    *dst = result;
...
FcBool FcStrBufString(FcStrBuf *buf, const FcChar8 *s) {
    FcChar8 c;
    while ((c = *s++))
        if (!FcStrBufChar(buf, c)) return FcFalse;
    return FcTrue;
}
...
FcBool FcStrUsesHome(const FcChar8 *s) { return *s == '~'; }
...
FcChar8 *FcStrBuildFilename(const FcChar8 *path, ...) {
    ...
    if (!FcStrSetAdd(sset, path)) goto bail0;

    va_start(ap, path);
    while (1) {
        s = (FcChar8 *)va_arg(ap, FcChar8 *);
        if (!s) break;
        if (!FcStrSetAdd(sset, s)) goto bail1;
    }
...
FcChar8 *FcStrDirname(const FcChar8 *file) {
    ...
    if (!slash) return FcStrCopy((FcChar8 *)".");
...
FcChar8 *FcStrBasename(const FcChar8 *file) {
    ...
    slash = FcStrLastSlash(file);
    if (!slash) return FcStrCopy(file);
...
FcChar8 *FcStrCanonFilename(const FcChar8 *s) {
    ...
    if (s[0] == '/')
        ...
        if (getcwd((char *)cwd, FC_MAX_FILE_LEN) == NULL) return NULL;
...
static FcBool _FcStrSetAppend(FcStrSet *set, FcChar8 *s) {
    ...
    if (set->num == set->size) {
        ...
        if (!strs) return FcFalse;
        if (set->num) memcpy(strs, set->strs, set->num * sizeof(FcChar8 *));
        if (set->strs) free(set->strs);
        set->size = set->size + 1;
        set->strs = strs;
    }
    set->strs[set->num++] = s;
...
FcBool FcStrSetMember(FcStrSet *set, const FcChar8 *s) {
    ...
    for (i = 0; i < set->num; i++)
        if (!FcStrCmp(set->strs[i], s)) return FcTrue;
    return FcFalse;
}

FcBool FcStrSetEqual(FcStrSet *sa, FcStrSet *sb) {
    int i;
    if (sa->num != sb->num) return FcFalse;
    for (i = 0; i < sa->num; i++)
        if (!FcStrSetMember(sb, sa->strs[i])) return FcFalse;
    return FcTrue;
}
...
static FcStrSet *FcStrSetReference(FcStrSet *set) {
    if (FcRefIsConst(&set->ref)) return set;
...
void FcStrSetDestroy(FcStrSet *set) {
    ...
    if (FcRefIsConst(&set->ref)) return;

    if (FcRefDec(&set->ref) != 1) return;

    for (i = 0; i < set->num; i++) FcStrFree(set->strs[i]);
    if (set->strs) free(set->strs);
    free(set);
}
...
void FcStrListFirst(FcStrList *list) { list->n = 0; }

FcChar8 *FcStrListNext(FcStrList *list) {
    if (list->n >= list->set->num) return 0;
