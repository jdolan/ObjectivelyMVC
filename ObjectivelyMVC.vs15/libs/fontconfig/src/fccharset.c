...
FcCharSet *FcCharSetNew(void) { return FcCharSetCreate(); }
...
void FcCharSetDestroy(FcCharSet *fcs) {
    ...
    if (fcs) {
        ...
        if (FcRefDec(&fcs->ref) != 1) return;
        for (i = 0; i < fcs->num; i++) free(FcCharSetLeaf(fcs, i));
        if (fcs->num) {
...
static int FcCharSetFindLeafForward(const FcCharSet *fcs, int start,
    ...
    while (low <= high) {
        int mid = (low + high) >> 1;
        page = numbers[mid];
        if (page == num) return mid;
        if (page < num)
            low = mid + 1;
        else
            high = mid - 1;
    }
    if (high < 0 || (high < fcs->num && numbers[high] < num)) high++;
    return -(high + 1);
}
...
static FcCharLeaf *FcCharSetFindLeaf(const FcCharSet *fcs, FcChar32 ucs4) {
    ...
    if (pos >= 0) return FcCharSetLeaf(fcs, pos);
    return 0;
}
    ...
    if (FC_IS_ZERO_OR_POWER_OF_TWO(fcs->num)) {
        if (!fcs->num) {
            ...
            if (new_leaves && distance) {
                int i;
                for (i = 0; i < fcs->num; i++) new_leaves[i] -= distance;
            }
            leaves = new_leaves;
        }

        if (!leaves || !numbers) return FcFalse;
...
FcBool FcCharSetAddChar(FcCharSet *fcs, FcChar32 ucs4) {
    ...
    if (fcs == NULL || FcRefIsConst(&fcs->ref)) return FcFalse;
    leaf = FcCharSetFindLeafCreate(fcs, ucs4);
    if (!leaf) return FcFalse;
...
FcBool FcCharSetEqual(const FcCharSet *a, const FcCharSet *b) {
    ...
    if (!a || !b) return FcFalse;
    for (FcCharSetIterStart(a, &ai), FcCharSetIterStart(b, &bi);
         ai.leaf && bi.leaf;
         FcCharSetIterNext(a, &ai), FcCharSetIterNext(b, &bi)) {
        if (ai.ucs4 != bi.ucs4) return FcFalse;
        for (i = 0; i < 256 / 32; i++)
            if (ai.leaf->map[i] != bi.leaf->map[i]) return FcFalse;
    }
    return ai.leaf == bi.leaf;
}

static FcBool FcCharSetAddLeaf(FcCharSet *fcs, FcChar32 ucs4,
                               FcCharLeaf *leaf) {
...
static FcCharSet *FcCharSetOperate(const FcCharSet *a, const FcCharSet *b,
    ...
    while ((ai.leaf || (bonly && bi.leaf)) && (bi.leaf || (aonly && ai.leaf))) {
        if (ai.ucs4 < bi.ucs4) {
            if (aonly) {
                if (!FcCharSetAddLeaf(fcs, ai.ucs4, ai.leaf)) goto bail1;
            ...
            if (bonly) {
                if (!FcCharSetAddLeaf(fcs, bi.ucs4, bi.leaf)) goto bail1;
            ...
            if ((*overlap)(&leaf, ai.leaf, bi.leaf)) {
                if (!FcCharSetAddLeaf(fcs, ai.ucs4, &leaf)) goto bail1;
            }
            FcCharSetIterNext(a, &ai);
            FcCharSetIterNext(b, &bi);
        }
...
static FcBool FcCharSetIntersectLeaf(FcCharLeaf *result, const FcCharLeaf *al,
    ...
    for (i = 0; i < 256 / 32; i++)
        if ((result->map[i] = al->map[i] & bl->map[i])) nonempty = FcTrue;
    return nonempty;
}
...
static FcBool FcCharSetUnionLeaf(FcCharLeaf *result, const FcCharLeaf *al,
    ...
    for (i = 0; i < 256 / 32; i++) result->map[i] = al->map[i] | bl->map[i];
    return FcTrue;
}
...
FcBool FcCharSetMerge(FcCharSet *a, const FcCharSet *b, FcBool *changed) {
    ...
    if (FcRefIsConst(&a->ref)) {
        if (changed) *changed = FcFalse;
        return FcFalse;
    }

    if (changed) {
        *changed = !FcCharSetIsSubset(b, a);
        if (!*changed) return FcTrue;
    }
    ...
    while (bi < b->num) {
        ...
        if (an < bn) {
            ...
            if (bn < an) {
                if (!FcCharSetAddLeaf(a, bn << 8, bl)) return FcFalse;
            } else {
    ...
    if (a && b) {
        ...
        while (ai.leaf && bi.leaf) {
            if (ai.ucs4 == bi.ucs4) {
                ...
                int i = 256 / 32;
                while (i--) count += FcCharSetPopCount(*am++ & *bm++);
                FcCharSetIterNext(a, &ai);
...
FcChar32 FcCharSetCount(const FcCharSet *a) {
    ...
    if (a) {
        for (FcCharSetIterStart(a, &ai); ai.leaf; FcCharSetIterNext(a, &ai)) {
            ...
            while (i--) count += FcCharSetPopCount(*am++);
        }
    }
    return count;
}
...
FcChar32 FcCharSetNextPage(const FcCharSet *a,
    ...
    if (!a) return FC_CHARSET_DONE;
    ai.ucs4 = *next;
    FcCharSetIterSet(a, &ai);
    if (!ai.leaf) return FC_CHARSET_DONE;
 ...
 *
 * Each leaf is represented as 9 32-bit values, the code of the first character
 *followed
 * by 8 32 bit values for the leaf itself.  Each value is encoded as 5 ASCII
 *characters,
 * only 85 different values are used to avoid control characters as well as the
 *other
 * characters used to encode font names.  85**5 > 2^32 so things work out, but
 * it's not exactly human readable output.  As a special case, 0 is encoded as a
 *space
 */
...
static FcChar8 *FcCharSetParseValue(FcChar8 *string, FcChar32 *value) {
    int i;
    FcChar32 v;
    FcChar32 c;
    ...
    if (*string == ' ') {
        ...
        v = 0;
        for (i = 0; i < 5; i++) {
            if (!(c = (FcChar32)(unsigned char)*string++)) return 0;
            c = charToValue[c];
            if (c == 0xff) return 0;
            v = v * 85 + c;
        }
...
static FcBool FcCharSetUnparseValue(FcStrBuf *buf, FcChar32 value) {
    int i;
    if (value == 0) {
        ...
        for (i = 0; i < 5; i++)
            if (!FcStrBufChar(buf, *s++)) return FcFalse;
    }
    return FcTrue;
}
...
FcCharSet *FcNameParseCharSet(FcChar8 *string) {
    ...
    while (*string) {
        ...
        if (bits) {
            ...
            if (!leaf) goto bail1;
            *leaf = temp;
            if (!FcCharSetInsertLeaf(c, ucs4, leaf)) goto bail1;
        }
    }
    return c;
bail1:
...
FcBool FcNameUnparseCharSet(FcStrBuf *buf, const FcCharSet *c) {
    ...
    for (FcCharSetIterStart(c, &ci); ci.leaf; FcCharSetIterNext(c, &ci)) {
        if (!FcCharSetUnparseValue(buf, ci.ucs4)) return FcFalse;
        for (i = 0; i < 256 / 32; i++)
            if (!FcCharSetUnparseValue(buf, ci.leaf->map[i])) return FcFalse;
    }
#ifdef CHECK
    {
        FcCharSet *check;
...
static FcCharLeafEnt *FcCharLeafEntCreate(FcCharSetFreezer *freezer) {
    if (!freezer->leaf_remain) {
        ...
        if (!freezer->current_block) return 0;
...
static FcCharSet *FcCharSetFreezeBase(FcCharSetFreezer *freezer,
                                      FcCharSet *fcs) {
...
static const FcCharSet *FcCharSetFindFrozen(FcCharSetFreezer *freezer,
    ...
    for (ent = *bucket; ent; ent = ent->next)
        if (ent->orig == orig) return ent->frozen;
    return NULL;
}
...
bail1:
    if (b->num) free(FcCharSetLeaves(b));
    if (b->num) free(FcCharSetNumbers(b));
    free(b);
bail0:
    return n;
}
...
FcBool FcCharSetSerializeAlloc(FcSerialize *serialize, const FcCharSet *cs) {
    ...
    if (!FcRefIsConst(&cs->ref)) {
        if (!serialize->cs_freezer) {
            ...
            if (!serialize->cs_freezer) return FcFalse;
        }
        if (FcCharSetFindFrozen(serialize->cs_freezer, cs)) return FcTrue;
    ...
    if (!FcSerializeAlloc(serialize, cs, sizeof(FcCharSet))) return FcFalse;
...
FcCharSet *FcCharSetSerialize(FcSerialize *serialize, const FcCharSet *cs) {
    ...
    if (cs->num) {
        ...
        if (!leaves_serialized) return NULL;
        ...
        for (i = 0; i < cs->num; i++) {
            ...
            if (!leaf_serialized) return NULL;
