...
void FcObjectSetDestroy(FcObjectSet *os) {
    ...
    if (os->objects) {
        for (i = 0; i < os->nobject; i++) FcFree(os->objects[i]);
                                      ...
                                      FcValueListPtr fntOrig) /* font */
{
    ...
    for (pat = patOrig; pat != NULL; pat = FcValueListNext(pat)) {
        ...
        if (fnt == NULL) return FcFalse;
    }
    return FcTrue;
}

static FcBool FcListValueListEqual(FcValueListPtr v1orig,
    ...
    for (v2 = v2orig; v2 != NULL; v2 = FcValueListNext(v2)) {
        for (v1 = v1orig; v1 != NULL; v1 = FcValueListNext(v1))
            if (FcValueEqual(FcValueCanonicalize(&v1->value),
                             FcValueCanonicalize(&v2->value)))
                break;
        if (v1 == NULL) return FcFalse;
    }
    return FcTrue;
}

static FcBool FcListPatternEqual(FcPattern *p1, FcPattern *p2,
                                 FcObjectSet *os) {
    int i;
    FcPatternElt *e1, *e2;
...
static FcChar32 FcListPatternHash(FcPattern *font, FcObjectSet *os) {
    ...
    for (n = 0; n < os->nobject; n++) {
        ...
        if (e) h = h ^ FcListValueListHash(FcPatternEltValues(e));
    }
    return h;
}
...
static FcBool FcListAppend(FcListHashTable *table, FcPattern *font,
    ...
    if (!bucket) goto bail0;
    bucket->next = 0;
    bucket->hash = hash;
    bucket->pattern = FcPatternCreate();
    if (!bucket->pattern) goto bail1;
...
FcFontSet *FcFontSetList(FcConfig *config, FcFontSet **sets, int nsets,
    ...
    if (!config) {
        if (!FcInitBringUptoDate()) goto bail0;
    ...
    for (set = 0; set < nsets; set++) {
        ...
        for (f = 0; f < s->nfont; f++)
                                      ...
                                      s->fonts[f])) /* font */
            {
                ...
                if (!FcListAppend(&table, s->fonts[f], os, lang)) goto bail1;
            }
    }
#if 0
    {
    ...
    for (i = 0; i < FC_LIST_HASH_SIZE; i++)
        while ((bucket = table.buckets[i])) {
            if (!FcFontSetAdd(ret, bucket->pattern)) goto bail2;
...
bail0:
    if (destroy_os) FcObjectSetDestroy(os);
    return 0;
}
...
FcFontSet *FcFontList(FcConfig *config, FcPattern *p, FcObjectSet *os) {
    ...
    if (!config) {
        if (!FcInitBringUptoDate()) return 0;
