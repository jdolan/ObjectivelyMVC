...
const FcObjectType *FcNameGetObjectType(const char *object) {
    ...
    if (!id) return FcObjectLookupOtherTypeByName(object);
...
FcBool FcObjectValidType(FcObject object, FcType type) {
    ...
    if (t) {
        ...
        case FcTypeDouble:
        case FcTypeInteger:
            if (type == FcTypeDouble || type == FcTypeInteger) return FcTrue;
            break;
        case FcTypeLangSet:
            if (type == FcTypeLangSet || type == FcTypeString) return FcTrue;
            break;
        default:
            if (type == t->type) return FcTrue;
            break;
        }
        return FcFalse;
    }
...
static FcValue FcNameConvert(FcType type, FcChar8 *string) {
    ...
    switch ((int)v.type) {
    case FcTypeInteger:
        if (!FcNameConstant(string, &v.u.i)) v.u.i = atoi((char *)string);
        break;
    case FcTypeString:
        ...
        break;
    case FcTypeBool:
        if (!FcNameBool(string, &v.u.b)) v.u.b = FcFalse;
        break;
    case FcTypeDouble:
...
FcPattern *FcNameParse(const FcChar8 *name) {
    ...
    for (;;) {
        ...
        if (save[0]) {
            if (!FcPatternAddString(pat, FC_FAMILY, save)) goto bail2;
        }
        if (delim != ',') break;
    }
    if (delim == '-') {
        for (;;) {
            ...
            if (e != save) {
                if (!FcPatternAddDouble(pat, FC_SIZE, d)) goto bail2;
            }
            if (delim != ',') break;
        }
    }
...
static FcBool FcNameUnparseString(FcStrBuf *buf, const FcChar8 *string,
    ...
    while ((c = *string++)) {
        if (escape && strchr((char *)escape, (char)c)) {
            if (!FcStrBufChar(buf, escape[0])) return FcFalse;
        }
...
FcBool FcNameUnparseValueList(FcStrBuf *buf, FcValueListPtr v,
                              FcChar8 *escape) {
    while (v) {
        if (!FcNameUnparseValue(buf, &v->value, escape)) return FcFalse;
        if ((v = FcValueListNext(v)) != NULL)
            if (!FcNameUnparseString(buf, (FcChar8 *)",", 0)) return FcFalse;
    }
    return FcTrue;
}
