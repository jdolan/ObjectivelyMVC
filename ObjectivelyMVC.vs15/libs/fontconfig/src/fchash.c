...
static FcChar32 *FcHashInitSHA256Digest(void) {
    int i;
    ...
    for (i = 0; i < 8; i++) ret[i] = h[i];

    return ret;
}

static void FcHashComputeSHA256Digest(FcChar32 *hash, const char *block) {
...
static FcChar8 *FcHashSHA256ToString(FcChar32 *hash) {
    FcChar8 *ret = NULL;
...
FcChar8 *FcHashGetSHA256Digest(const FcChar8 *input_strings, size_t len) {
    ...
    /* padding */
    if ((len % 64) != 0) memcpy(block, &input_strings[len / 64], len % 64);
...
FcChar8 *FcHashGetSHA256DigestFromFile(const FcChar8 *filename) {
    ...
    if (FcStat(filename, &st)) goto bail0;
...
FcChar8 *FcHashGetSHA256DigestFromMemory(const char *fontdata, size_t length) {
    char ibuf[64];
    FcChar32 *ret;
    size_t i = 0;
    ...
    while (i <= length) {
        if ((length - i) < 64) {
            ...
            /* add a padding */
            n = length - i;
            if (n > 0) memcpy(ibuf, &fontdata[i], n);
